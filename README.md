...
| `wstate.py` | **Learned word memory, audited (§72–§77)** — the §72 "word-slot binding crosses the parity wall" result came from an instrument bug: the slot memory held word *prefixes*, so the win was a learned current-word model (one prefix slot beats six). Real whole-word slots lose to the orders baseline on code and text; the engine port had trained its vectors with the wrong sign (fixed: slots add nothing on text, +0.00004 bits/bit on code); content attention over 32 whole words fails a pre-registered long-gap binding probe. `WSLOTMODE=word` selects real word memory; the default keeps §72–§76 bit-identical. The learned-memory model track is closed (§77) |
