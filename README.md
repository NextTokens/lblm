...
| `wstate.py` | **The parity wall falls (§72); hard similarity fails honestly (§73)** — probe-driven arc: EMA-projection, exact trace-bank credit, and nonlinear readout all fail identically because the EMA *superposes* ~5 words and no readout can isolate one → **word-slot binding** (LRU of recent words, each holding its own 8-dim vector fed directly to the mixer, credit exact by construction) **crosses below the orders baseline at ≥1.2 MB with the margin growing in data, replicated across seeds**; frozen-identity slots never cross, so the win is the *vectors the compression loss teaches*. The strong.rs port (`BLMSLOTS=1`, default bit-identical) honestly adds nothing at 11 MB; §73: embedding-bucket/identity-keyed experts both lose at scale (hard quantisation destroys graded similarity) → the named next build is **soft retrieval in Rust** (LSH top-k over the vector table, similarity-weighted word-model votes) |
