...
| `wstate.py` | **The parity wall falls (§72)** — probe-driven arc: EMA-projection, exact trace-bank credit, and nonlinear readout all fail identically because the EMA *superposes* ~5 words and no readout can isolate one → **word-slot binding** (LRU of recent words, each holding its own 8-dim vector fed directly to the mixer, credit exact by construction) **crosses below the orders baseline at ≥1.2 MB with the margin growing in data, replicated across seeds**; frozen-identity slots never cross, so the win is the *vectors the compression loss teaches*. The strong.rs port (`BLMSLOTS=1`, default bit-identical) honestly adds nothing at 11 MB — identity is cheap at lpaq capacity; **similarity is the open lever (Phase 2)** |
