...
| `wstate.py` | **The parity wall falls (§72); hard similarity fails honestly (§73); soft retrieval measured inert in production (§74)** — probe-driven arc: EMA-projection, exact trace-bank credit, and nonlinear readout all fail identically because the EMA *superposes* ~5 words and no readout can isolate one → **word-slot binding** (LRU of recent words, each holding its own 8-dim vector fed directly to the mixer, credit exact by construction) **crosses below the orders baseline at ≥1.2 MB with the margin growing in data, replicated across seeds**; frozen-identity slots never cross, so the win is the *vectors the compression loss teaches*. §73: embedding-bucket/identity experts both lose at scale (hard quantisation destroys graded similarity). §74 (`strong.rs BLSOFT=1`, default bit-identical): LSH top-k + similarity-weighted word-model votes — all configs +0.0001 vs 0.217011 at 11 MB: three independent channels now proven redundant at lpaq capacity; the open questions are scale (enwik8 at 30–100 MB) and long-range-rich streams |
