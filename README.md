...
| `wstate.py` | **Learned word memory, audited (§72–§79)** — the §72 "word-slot binding crosses the parity wall" result came from an instrument bug: the slot memory held word *prefixes*, so the win was a learned current-word model. Real whole-word slots lose to the orders baseline on code and text, and the engine port had trained its vectors with the wrong sign (§77). That learned vector's gain turned out to be mostly *forgetting* the instrument's never-forgetting counts lack (§78); carried into the engine, forgetting in the word models plus the learned prefix vector (`strong.rs` `BLWNS=1 BLPVEC=2`, default OFF) takes full enwik8 from 0.199145 to 0.196123 bits/bit; decodability verified by a real encode/decode round trip on corpus_big (§79). Every learned selector tested (attention, cosine attention, a mixture-of-experts gate and its amendment, all querying from slot 0) failed a synthetic long-gap binding probe (§77–§79); a different query source is untested |
